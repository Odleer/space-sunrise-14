...
namespace Content.Shared._Sunrise.Factory;
...
/// </summary>
public abstract class SharedFactorySystem : EntitySystem
{
    ...
    {
        SubscribeLocalEvent<CollideFactoryComponent, StartCollideEvent>(OnCollide);
        SubscribeLocalEvent<FactoryComponent, GetVerbsEvent<Verb>>(OnGetVerb);
    }

    private void OnCollide(EntityUid uid, CollideFactoryComponent component, ref StartCollideEvent args)
    {
        // if (args.OurFixtureId != component.FixtureId)
        //     return;
        if (!TryComp<FactoryComponent>(uid, out var reclaimer))
            return;
        TryStartProcessItem(uid, args.OtherEntity, reclaimer);
    }

    /// <summary>
    /// Tries to start processing an item via a <see cref="FactoryComponent"/>.
    /// </summary>
    protected void TryStartProcessItem(EntityUid uid,
        EntityUid item,
        FactoryComponent? component = null,
        EntityUid? user = null,
        bool suicide = false)
    {
    ...
    /// </summary>
    protected virtual void Reclaim(EntityUid uid, EntityUid item, FactoryComponent? component = null)
    {
        if (!Resolve(uid, ref component))
            return;
    ...
    protected abstract void OnGetVerb(EntityUid uid, FactoryComponent component, GetVerbsEvent<Verb> args);
}